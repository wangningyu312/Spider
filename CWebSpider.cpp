...
bool CWebSpider::GetHttpResponse_Img(const string & strUrl, char* & pResponse, int & iBytesRead)
{
	string strHost;       ///主机
	string strResource;   ///资源
	if (!ParseURL(strUrl, strHost, strResource))
	...
	while (ret > 0)
	{
		ret = recv(sock, pageBuf + iBytesRead, m_nContentLength - iBytesRead, 0);
		...
		if (m_nContentLength - iBytesRead<100)
		{
			//cout << "\nRealloc memorry" << endl;
			m_nContentLength *= 2;
...
void CWebSpider::DownLoadImg(vector<string>& vecImgUrl, const string& strUrl)
{
	...
	for (int i = 0; i<vecImgUrl.size(); i++)
	{
		...
		//下载其中的内容  
		if (GetHttpResponse_Img(strImg, image, byteRead))
		{
			if (strlen(image) < 1) 
			{
				continue;
